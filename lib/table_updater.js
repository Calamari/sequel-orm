...
var TableUpdater = function(client, tableName, block, cb) {
  ...
  for (var i=0,l=this._addUniques.length; i<l; ++i) {
    var keyName = jaz.Object.isArray(this._addUniques[i]) ? this._addUniques[i].join('_') : this._addUniques[i];
    columnsSqls.push('ADD UNIQUE ' + keyName + ' (' + (jaz.Object.isArray(this._addUniques[i]) ? this._addUniques[i].join(',') : this._addUniques[i]) + ')');
  }
  for (var i=0,l=this._removeKeys.length; i<l; ++i) {
    var keyName = jaz.Object.isArray(this._removeKeys[i]) ? this._removeKeys[i].join('_') : this._removeKeys[i];
