...
var Model = function(name, structure, modelDefinition, query) {
    ...
    _find(search, userParams, function(err, results) {
      if (err) {
        ...
        if (results.length) {
          ...
          if (search.include) {
            var count = 0;
            search.include.forEach(function(include) {
              ++count;
              record[jaz.String.camelize('get_' + include)](function(err) {
                if (--count === 0) {
                  cb && cb(null, record);
                }
              });
  ...
  var _findAll = function(search, userParams, cb) {
    var createMethod = this.create;
    if (typeof search === 'function') {
      cb = search;
      userParams = [];
    ...
    _find(search, userParams, function(err, results) {
      if (err) {
        ...
        var hashedResult  = !!search.hash,
            records       = hashedResult ? {} : [],
            resultsLength = results.length,
            hashKey       = search.hash === true ? 'id' : search.hash,
            recordData, key, i;

        if (!hashedResult || structure[hashKey]) {
          if (resultsLength) {
          ...
          cb && cb(new errors.NotValidColumnError("Key '" + hashKey + "' is not a valid Column in table '" + modelNames.tableName + "'"));
        }
      }
    });
  };
   ...
   *      {String}       hash  Which column should be used as key for output as hash? (Defaults to false = result as Array)
   * @param {Integer|Object} [search]     id or search parameters
   * @param {Array}          [userParams] Array of replacements for ? in query
   * @param {function}       [cb]         Callback to call after query has finished
   */
  this.findAll = function(search, userParams, cb) {
    _findAll.call(this, search, userParams, cb);
  };
  ...
  this.create = function(data, loaded) {
    ...
    /**
     * Updates some attributes of the instance and saves it directly
     */
    instance.updateAttributes = function(attributes, cb) {
    ...
    instance.save = function(cb) {
      ...
      var columns = [],
          ...
          params  = [],
          sql, i, l,
          unsavedAssociations = false;

      // check for unsaved associations
      unsavedAssociations = Object.keys(loadedAssociations).some(function(modelName) {
      ...
      if (unsavedAssociations) { return; }
      unsavedAssociations = Object.keys(loadedManyAssociations).some(function(assocModelName) {
