...
var Model = function(name, structure, modelDefinition, query) {
  ...
  this.create = function(data, loaded) {
    ...
    instance.__defineGetter__('errors', function() { return errors; });
    ...
    instance.validate = function() {
      ...
      for (var column in structure) {
        if (structure[column].options && structure[column].options.required) {
          if (!data[column]) {
            ok = false;
            errors.push({ column: column, type: 'required' });
          }
        } else if (structure[column].validation && data[column] !== undef) {
          if (!structure[column].validation(data[column])) {
            ok = false;
            errors.push({ column: column, type: 'default' });
          }
        }
      }
      return ok;
