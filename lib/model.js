...
var Model = function(name, structure, modelDefinition, query) {
  var modelNames          = {
      ...
      fields,
      hookCallbacks       = {},
      oneAssociations     = { hasOne: {}, belongsTo: {} },
      hasManyAssociations = {},
      thisModel           = this;
  ...
  function addOneAssociation(model, type) {
    var self      = this,
        SequelORM = require(__dirname + '/');
    if (typeof model !== 'string') {
      oneAssociations[type][model.name] = model;
    } else {
      var definedModel = SequelORM.getModel(model);
      if (definedModel) {
        oneAssociations[type][model] = definedModel;
      } else {
  ...
  this.belongsTo = function(model) {
    addOneAssociation(model, 'belongsTo');
  };
  ...
  this.hasOne = function(model) {
    addOneAssociation(model, 'hasOne');
  };
  ...
  var _find = function(search, userParams, cb) {
     ...
     */
    if (Object.keys(oneAssociations['hasOne']).length) {
      Object.keys(oneAssociations['hasOne']).forEach(function(modelName) {
        var assoc          = hasManyAssociations[modelName],
            assocTableName = SequelORM.getModel(modelName).tableName;
  ...
  this.create = function(data, loaded) {
    ...
    function prepareOneAssociations(type) {
      Object.keys(oneAssociations[type]).forEach(function(modelName) {
        var model      = oneAssociations[type][modelName],
    ...
    instance.save = function(cb) {
      ...
      Object.keys(loadedAssociations).forEach(function(modelName) {
        var isBelongsTo = !!oneAssociations.belongsTo[modelName];
        if (isBelongsTo && !loadedAssociations[modelName].id) {
          allAssociationsSaved = false;
          cb(new errors.AssociationsNotSavedError("Association '" + modelName + "' was not saved."));
        }
      });
