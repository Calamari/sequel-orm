...
var Model = function(name, structure, modelDefinition, query) {
  ...
  var _find = function(search, userParams, cb) {
    ...
    if (search.only) {
      selects.push(modelNames.tableName + '.id');
      // check attributes for validity
      for (var i = search.only.length; i--;) {
        var attr = search.only[i];
        if (structure[attr]) {
          selects.push(modelNames.tableName + '.' + attr);
        } else {
      ...
      selects.push(modelNames.tableName + '.*');
    }
    /**
