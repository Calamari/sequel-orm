...
module.exports['models'] = {
  ...
  'test finding a model': function(test) {
    ...
    Seq.on('log', function(status, message) {
      test.equal(status, Seq.LOG_LEVEL_INFO);
      test.equal(message, "Sending SQL: SELECT products.* FROM `products` products  WHERE products.id=?   LIMIT 1;");
    });
    var Product = Seq.getModel('Product');
    Product.find(1);
    test.done();
  ...
  'test finding several models': function(test) {
    ...
    Seq.on('log', function(status, message) {
      test.equal(status, Seq.LOG_LEVEL_INFO);
      test.equal(message, "Sending SQL: SELECT products.* FROM `products` products  WHERE name='Bob'   LIMIT 4,1;");
    });
    var Product = Seq.getModel('Product');
